...
export default function Moodboard2() {
        ...
        <div className='moodboard-2'>
            ...
            <ul>
                {items.map((item, index) => {
                    ...
                    if (item.type === 'color') {
                        display = (
                            <li key={index} className="color-box" style={{ backgroundColor: item.content }}>New Color <button onClick={() => deleteItem(index)}>X</button></li>
                        )
                        return display;
                    }
                    if (item.type === 'image') {
                        display = (
                            <li key={index} className="color-box" ><img src={item.content}></img> <button onClick={() => deleteItem(index)}>X</button></li>
                        )
                        return display;
                    }
                    if (item.type === 'link') {
